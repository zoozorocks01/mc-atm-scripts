...
"$JBIN/javac" --release 21 -proc:none -cp "$CP" -d build/classes src/dev/zjn/simpleafklookguard/mixin/*.java
"$JBIN/javac" --release 21 -proc:none -cp "build/classes:$CP" -d build/test-classes test/dev/zjn/simpleafklookguard/mixin/*.java
"$JBIN/java" -cp "build/classes:build/test-classes:$CP" dev.zjn.simpleafklookguard.mixin.LookAngleThresholdTest
cp -R build/classes/. build/jar/
cp -R resources/. build/jar/
"$JBIN/jar" --create --file "build/simpleafk-look-guard-$VERSION.jar" -C build/jar .
echo "built: build/simpleafk-look-guard-$VERSION.jar"
