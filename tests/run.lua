...
do
  ...
  t.eq(control.unsettledJobs(bridge, out), 1, "unsettledJobs: only the live job blocks (done/errored/purged settle)")
  t.eq(control.unsettledJobs(bridge, {}), 0, "unsettledJobs: no recorded jobs -> 0")
  t.eq(control.unsettledJobs(bridge, nil), 0, "unsettledJobs: missing outstanding list -> 0 (old craftstate)")
  t.eq(control.unsettledJobs({}, out), nil, "unsettledJobs: recorded jobs but no getCraftingTask API -> nil (blind)")
  t.eq(control.unsettledJobs(nil, out), nil, "unsettledJobs: no bridge -> nil (blind)")
  local throwing = { getCraftingTask = function() error("boom") end }
  t.eq(control.unsettledJobs(throwing, { { id = 1 } }), 0, "unsettledJobs: bridge throw on lookup counts as settled (pcall)")
  local methodless = { getCraftingTask = function() return {} end } -- job with NO status methods
  t.eq(control.unsettledJobs(methodless, { { id = 1 } }), 1, "unsettledJobs: unqueryable job state stays conservative (unsettled)")
end
